  ...
  return (
    <div>
      <div className="mb-1 text-xs text-zinc-500">
        <Link href="/library?tab=videos" className="hover:text-zinc-800">
          Videos
        </Link>{" "}
        / {v.filename}
      </div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="accent" onClick={() => router.push(`/create?source=video&id=${id}`)}>
              Create from this
            </Button>
          ...
          {v.content.length ? (
            <Panel title="Content from this video">
              <ul className="space-y-1 text-[13px]">
                {v.content.map((c) => (
                  <li key={c.id}>
                    <Link href={`/library/content/${c.id}`} className="text-zinc-900 hover:text-accent-strong">
                      {c.title}
                    </Link>{" "}
  ...
  return (
    <div id={`clip-${clip.id}`} className={cn("rounded-md border bg-white", clip.status === "dismissed" ? "border-zinc-200 opacity-60" : "border-zinc-200")}>
      ...
      <div className="flex flex-wrap items-center gap-1.5 border-t border-zinc-200 px-3 py-2">
        ...
        {contentId ? (
          <Link href={`/library/content/${contentId}`} className="text-xs text-accent-strong hover:underline">
            Draft created — open
          </Link>
        ) : null}
