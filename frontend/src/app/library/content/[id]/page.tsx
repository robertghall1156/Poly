...
import { cn, fmtDateTime, fmtNumber, humanize, labelFormat, labelStatus, relTime } from "@/lib/utils";
...
export default function ContentDetailPage() {
  ...
  const studio = useApi(() => api.studioByContent(id).catch(() => null), [id]);
  ...
  const remove = async () => {
    ...
    if (ok !== undefined) router.push("/library?tab=content");
  };
  ...
  return (
    <div>
      <div className="mb-1 text-xs text-zinc-500">
        <Link href="/library?tab=content" className="hover:text-zinc-800">
          Library
        </Link>
        {c.lineage.parents.map((p) => (
          <span key={p.id}>
            {" / "}
            <Link href={`/library/content/${p.id}`} className="hover:text-zinc-800">
              {p.title}
            </Link>
          </span>
        ))}
      ...
      <PageHeader
        ...
        description={
          <span className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1">
              <FactCheckDot status={c.fact_check_status} /> {labelStatus(c.fact_check_status)}
            </span>
        ...
        actions={
          <>
            {studio.data ? (
              <>
                <Button variant="default" onClick={() => router.push(`/create/studio/${studio.data!.id}`)}>
                  Open in studio
                </Button>
                <Button variant="accent" onClick={() => router.push(`/create/review/${studio.data!.id}`)}>
                  Review
                </Button>
              </>
            ) : null}
            <Button onClick={() => router.push(`/create?source=story&id=${c.story_id ?? ""}`)} title="Start something new from this draft's story" disabled={!c.story_id}>
              Create from this
            </Button>
            <Button onClick={() => setBook(true)}>Save for book</Button>
            <Button onClick={() => setGen(true)}>Spin off a version</Button>
            <Button variant={cloud ? "warn" : "secondary"} onClick={runSocial} loading={act.busy}>
              Make social versions
            </Button>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0">
          ...
          {tab === "tree" ? (
            <Panel title="Family tree" actions={<span className="text-xs text-zinc-400">Lineage from the root item down to every derivative.</span>}>
          ...
          <Panel title="Lineage">
            <dl className="space-y-1.5 text-[13px]">
              <div>
                ...
                <dd>
                  {c.lineage.story ? (
                    <Link href={`/discover/stories/${c.lineage.story.id}`} className="text-zinc-900 hover:text-accent-strong">
              ...
              <div>
                <dt className="text-[11px] uppercase text-zinc-500">My position</dt>
                <dd>
                  {c.lineage.brief ? (
                    <Link href={`/think/positions/${c.lineage.brief.id}`} className="text-zinc-900 hover:text-accent-strong">
              ...
              <div>
                <dt className="text-[11px] uppercase text-zinc-500">Beliefs</dt>
                <dd>
              ...
              {c.lineage.source_video_id ? (
                <div>
                  ...
                  <dd>
                    <Link href={`/library/videos/${c.lineage.source_video_id}${c.lineage.clip_id ? `#clip-${c.lineage.clip_id}` : ""}`} className="text-zinc-900 hover:text-accent-strong">
                      Open source video
                    </Link>
                  </dd>
                </div>
              ...
              {c.lineage.parents.length ? (
                <div>
                  ...
                  <dd>
                    <Link href={`/library/content/${c.lineage.parents[0].id}`} className="text-zinc-900 hover:text-accent-strong">
                  ...
                  <ul>
                    {c.children.map((k) => (
                      <li key={k.id} className="flex items-center gap-1.5">
                        <Link href={`/library/content/${k.id}`} className="min-w-0 flex-1 truncate text-zinc-900 hover:text-accent-strong">
                          {k.title}
                        </Link>
  ...
  return (
    <div>
      <div className={cn("flex items-center gap-2 rounded px-1.5 py-1", node.id === current && "bg-accent-soft")} style={{ marginLeft: depth * 18 }}>
        ...
        <Link href={`/library/content/${node.id}`} className="min-w-0 flex-1 truncate text-[13px] font-medium text-zinc-900 hover:text-accent-strong">
          {node.title}
        </Link>
