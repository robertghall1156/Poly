  ...
  return (
    <div>
      <div className="mb-1 text-xs text-zinc-500">
        <Link href="/principles" className="hover:text-zinc-800">
          Principles
        </Link>{" "}
        / {d.category}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="space-y-3">
            ...
            <ul className="space-y-1.5">
              {d.stories.map((s) => (
                <li key={s.story_id} className="text-[13px]">
                  <Link href={`/stories/${s.story_id}`} className="text-zinc-900 hover:text-accent-strong">
                    {s.title}
                  </Link>
          ...
          <Panel title="Position briefs">
            {d.briefs.length === 0 ? <p className="text-xs text-zinc-400">None.</p> : null}
            <ul className="space-y-1">
              {d.briefs.map((b) => (
                <li key={b.id} className="flex items-center gap-2 text-[13px]">
                  <Link href={`/think/briefs/${b.id}`} className="min-w-0 flex-1 truncate text-zinc-900 hover:text-accent-strong">
                    {b.issue}
                  </Link>
