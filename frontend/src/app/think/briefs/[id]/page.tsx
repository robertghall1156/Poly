    ...
    <div>
      ...
      <PageHeader
        ...
        description={
          <span className="flex flex-wrap items-center gap-2">
            ...
            {b.story_id ? (
              <Link href={`/stories/${b.story_id}`} className="text-accent-strong hover:underline">
                Open story
              </Link>
            ) : null}
          </span>
        ...
        actions={
          <>
            <Button onClick={() => setGen(true)}>Create content from this brief</Button>
      ...
      {b.status === "approved" && approvedPrinciple ? (
        <Notice kind="success" className="mb-3">
          Approved into principle{" "}
          <Link href={`/principles/${approvedPrinciple.id}`} className="font-medium underline">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <aside className="space-y-3">
          <Panel title="Governing principle">
            ...
            {governing ? (
              <Link href={`/principles/${governing.id}`} className="mt-1 block text-xs text-accent-strong hover:underline">
                Open principle
              </Link>
            ) : null}
          </Panel>
      ...
      />
      <GenerateContentDialog open={gen} onClose={() => setGen(false)} defaults={{ brief_id: b.id, story_id: b.story_id, principle_ids: b.governing_principle_id ? [b.governing_principle_id] : [], format: "youtube" }} />
    </div>
  );
}
...
function ApproveDialog({ open, onClose, brief, onApproved }: { open: boolean; onClose: () => void; brief: PositionBrief; onApproved: () => void }) {
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title="Approve into the Political Operating System"
      footer={
        <>
    ...
    >
      <div className="space-y-3">
        <p className="text-xs text-zinc-600">Approving writes this position into a principle. Every position change records a revision with your reason.</p>
        <Field label="Mode">
          <Select value={mode} onChange={(e) => setMode(e.target.value)} className="w-full">
            <option value="auto">Auto — revise the governing principle if set, otherwise create a new one</option>
            <option value="revise">Revise an existing principle</option>
            <option value="new">Create a new principle</option>
          </Select>
        </Field>
        {mode === "revise" ? (
          <Field label="Principle to revise">
        ...
        <Field label="Reason for change">
          <Textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Why does this position deserve to enter (or change) the operating system?" />
        </Field>
        <ErrorNotice error={act.error} />
        {result ? (
          <Notice kind="success">
            Approved.{" "}
            <Link href={`/principles/${result.id}`} className="font-medium underline">
              Open principle “{result.title}”
            </Link>
          </Notice>
        ) : null}
      </div>
