...
function ResearchInner() {
  ...
  const [tab, setTab] = React.useState(params.get("tab") ?? "notes");
  return (
    <div>
      <PageHeader title="Research" description="Notes attached to stories and principles, source reliability, and the feeds that drive ingest." />
      <Tabs
        className="mb-4"
        value={tab}
        onChange={setTab}
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <Select value={principleId} onChange={(e) => setPrincipleId(e.target.value)} className="max-w-xs">
          <option value="">Any principle</option>
        ...
        {(notes.data ?? []).map((n) => (
          <div key={n.id} className="rounded-md border border-zinc-200 bg-white px-3 py-2">
            ...
            <div className="mt-1 flex flex-wrap gap-3 text-xs text-zinc-500">
              {n.story_id ? (
                <Link href={`/stories/${n.story_id}`} className="hover:text-accent-strong">
                  Story: {sTitle(n.story_id) ?? n.story_id.slice(0, 8)}
                </Link>
              ) : null}
              {n.principle_id ? (
                <Link href={`/principles/${n.principle_id}`} className="hover:text-accent-strong">
                  Principle: {pTitle(n.principle_id) ?? n.principle_id.slice(0, 8)}
                </Link>
              ) : null}
              {n.content_item_id ? (
                <Link href={`/content/${n.content_item_id}`} className="hover:text-accent-strong">
                  Content item
                </Link>
              ) : null}
            </div>
  ...
  return (
    <div className="mb-4 rounded-md border border-zinc-300 bg-white p-3">
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <Field label="Principle" className="md:col-span-2">
