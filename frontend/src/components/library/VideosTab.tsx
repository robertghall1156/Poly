...
export function VideosTab() {
  ...
  return (
    <div>
      <Notice className="mb-4">Point Poly at a folder of your videos below. They are indexed and transcribed locally — nothing leaves this machine.</Notice>
            ...
            <TBody>
              {videos.data.map((v) => (
                <TR key={v.id} className={v.missing ? "opacity-60" : ""}>
                  <TD>
                    <Link href={`/library/videos/${v.id}`} className="font-medium text-zinc-900 hover:text-accent-strong">
                      {v.filename}
                    </Link>
