  ...
  return (
    <div
      className={cn(
        ...
        status === "failed" ? "border-danger/40 bg-danger-soft" : status === "succeeded" ? "border-accent/40 bg-accent-soft" : cloud ? "border-warn/50 bg-warn-soft" : "border-zinc-200 bg-zinc-50",
        className,
      )}
    >
      <div className="flex items-center gap-2">
        <span className="font-medium text-zinc-800">{label ?? humanize(job?.kind ?? "job")}</span>
        <span className={cn("rounded px-1 text-[10px] uppercase tracking-wide", status === "failed" ? "bg-danger-soft text-danger" : status === "succeeded" ? "bg-accent-soft text-accent-strong" : "bg-zinc-200 text-zinc-700")}>{status}</span>
        {cloud ? <span className="bg-highlight px-1 text-[10px] uppercase text-ink">cloud</span> : null}
      ...
      {job?.error ? <p className="mt-1 break-words text-danger">{job.error}</p> : null}
      {error ? <p className="mt-1 text-danger">Polling error: {error}</p> : null}
    </div>
  );
}
